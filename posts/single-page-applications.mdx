...
excerpt: 'Learn what is a Single Page Applications, how they work, and how they differ from traditional websites.'
---
...
Any time the user clicks on an internal link the url changes and a new page is requested to the server. The server responds with an HTML file which contains exactly the content of the requested page, which is then rendered by the browser.
...
This is possible thanks to JavaScript. Single Page Applications contain a lot of JavaScript code which is responsible for creating the elements which compose the page, and reorganize them to respond to user actions.

For example, when the user clicks on a link JavaScript observes a change in the URL and modifies the content of the page to show the content of the corresponding section of the website.
...
Instead we talk about Server Side Rendering when is the server that generates the HTML that makes up the page, and sends it to the client ready to be rendered.
...
React is my personal favorite choice and the React team has also made a tool called [Create React App](https://create-react-app.dev/), which let's you create a SPA very quickly and with zero configuration.
...
In traditional websites is the server which takes care of this. When the page is requested by the client, the server queries the database (or whatever the source of the data is) and populates the page with the obtained data. Then it sends the page to the client, which only has to concern about rendering it.

In a Single Page Applications this process is JavaScript's job again. The page obtained by the server doesn't contain data. It is instead JavaScript which **makes another request to the server to get data** and uses the obtained information (typically in [JSON](https://it.wikipedia.org/wiki/JavaScript_Object_Notation) format) to generate the UI dynamically.
